...
class NGramCreator:
    ...
    def encode(self, kind, ngram, markov_id=None):
        if kind == 'ip_list':
            ...
            raw_seed = randint(0 if (alph_idx % self.alphabet_len) == 0 else self.cp_list[index - 1], self.cp_list[index]-1)
            range = self.cp_list[index] if (alph_idx % self.alphabet_len) == 0 else (self.cp_list[index] - self.cp_list[index - 1])
            fill = self.cp_list[index + (self.alphabet_len - (alph_idx % self.alphabet_len)) - 1]
            while True:
                seed = randint(0, 0xFFFF) * fill + raw_seed
    ...
    def decode_ip(self, seed, markov_id, idxonly=False):
        ...
        if idxonly:
            return idx[0]
        return self._i2nIP(idx[0]), (self.ip_list[idx[0]] - (0 if idx[0]==offset else self.ip_list[idx[0]-1])) / self.ip_list[(markov_id+1) * self.no_ip_ngrams-1]
    ...
    def decode_cp(self, seed, pre, markov_id):
        ...
        #assert idx[0] == idx_
        return self._i2nCP(idx[0]), (self.cp_list[idx[0]] - (0 if idx[0]==start else self.cp_list[idx[0]-1])) / self.cp_list[start+self.alphabet_len-1]
    ...
    def decode_len(self, seed):
        ...
        for x in self.length_dic:
            if seed < self.length_dic[x]:
                return x, (self.length_dic[x] - self.length_dic[x-1]) / self.length_dic[MAX_PW_LENGTH-self.seed_length_rec]
    ...
    def decode_pw(self, seed):
        ...
        prob_lst = []
        len_seed = seed[0]
        length, prob = self.decode_len(len_seed)
        ...
        if pw_len in TRAIN_LENGTH:
            ...
            markov_id = len(TRAIN_LENGTH)
        ip_seed = seed[1]
        ip, prob = self.decode_ip(ip_seed, markov_id)
        prob_lst.append(prob)
        pw = ip
        # print(length)
        for x in range(2, length + 1):
            cp, prob = self.decode_cp(seed[x], pre=pw[-self.ngram_size + 1:], markov_id=markov_id)
            prob_lst.append(prob)
            pw += cp[-1]
        return pw, np.log(np.array(prob_lst)).sum()

    def decode_pw_continue(self, seed, pw, target_len):
        prob_lst = []
        pw_len = target_len
        if pw_len in TRAIN_LENGTH:
            markov_id = TRAIN_LENGTH.index(pw_len)
            ...
            markov_id = len(TRAIN_LENGTH)
        # print(length)
        original_length = len(pw)
        for x in range(target_len - original_length):
            cp, prob = self.decode_cp(seed[x], pre=pw[-self.ngram_size + 1:], markov_id=markov_id)
            pw += cp[-1]
            prob_lst.append(prob)
        return pw, np.log(np.array(prob_lst)).sum()